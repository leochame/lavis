...
> 本文档记录了 Lavis 项目的开发计划和实现状态，作为历史参考。

---

## 📋 开发计划概述

本文档记录了以下三项核心改进方案的开发计划和实现状态：
...
### 目标
在任务计划全部完成时（最终步骤），向前端推送简短的拟人化语音反馈，通知用户任务已完成，同时确保不阻塞自动化任务的执行速度。
...
### 目标
实现"听觉唤醒，视觉克制"。语音唤醒时不直接占据屏幕，仅通过胶囊组件的动效反馈，减少对用户的干扰。
...
### 目标
防止长时间运行导致内存泄漏或 OOM（OutOfMemory）。策略覆盖浏览器端、Electron 进程通信及 Java 后端堆内存。
...
- ✅ **Store 瘦身**：`voiceStore` 中的 Base64 音频数据在播放开始后立即清理
...
| IPC/后端内存 | 大对象管理 | 历史截图淘汰机制 + 上下文及时清理 |
