 ...
 * 包括：Markdown 标记、代码块、URL、特殊符号etc
 *
 * 设计原则：
 * 1. 清洗后的文本should是自然语言，适合语音朗读
 * 2. if清洗后为空，表示该回复不适合语音播放
 */
@Slf4j
@Service
public class TextCleanerService {
     ...
     * @return 清洗后的文本，if不适合朗读则返回空characters符串
     */
    ...
    /**
     * will 长文本分割为句子columns表（用于流式 TTS）
     *
     * @param text 清洗后的文本
     * @return 句子columns表
     */
    ...
    /**
     * 检查文本是否包含可朗读的内容（不只是符号和数characters）
     */
    private boolean containsReadableContent(String text) {
        // 至少包含一items中文characters符或英文characters母
        return text.matches(".*[\\u4e00-\\u9fa5a-zA-Z].*");
    }

    /**
     * 快速检查文本是否主要是代码
     * 用于在清洗前快速判断是否need跳过 TTS
     *
     * @param text 原始文本
