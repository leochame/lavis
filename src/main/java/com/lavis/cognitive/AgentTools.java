...
public class AgentTools {
     ...
     *
     * 用途：
     * - 在「start执lines前」先梳理整体思路和步骤规划。
     * - 在「执lines过程中」对when前进展进lines反思、调整策略。
     * - 在「出现exception或卡住」时，总结问题、提出假设和下一步计划。
     *
     * 设计约束：
     * - 本工具绝不调用任何外部系统，不做点击/输入/网络请求etc操作，仅在内部记录一段文characters。
     * - 每times调用时，入参中的思考内容会被**原样作为 tool_result 返回**，便于编排器/上层系统记录和展示思考链路。
     *
     * 使用建议（对 LLM）：
     * - when任务复杂、多步骤或存在不确定性时，优先调用本工具进lines显式规划。
     * - 反思内容应尽量结构化，for example：
     *   1) when前目标
     *   2) has been 知info
     *   3) 风险与不确定点
     *   4) 接下来 1~3 步的具体lines动计划（对应可调用的工具）
     */
    ...
    /**
     * will  Gemini 归一化坐标 (0-999) 转为 macOS AWT Robot 使用的逻辑屏幕坐标 (points)。
     * 说明：
     * - 屏幕截图叠加网格/模型输出使用 Gemini 坐标系 (0-999)；
     * - Java 9+ macOS 下 AWT Robot 使用logical coordinates，不是物理像素；
     * - therefore这里need做"坐标系转换"，而不是乘以 Retina 缩放因子。
     * 
    ...
    public String click(@P("Coordinate position array [x, y] in Gemini format (0-999)") int[] coords) {
        ...
        try {
            ...
            // 关键修改：不再仅仅说"success"，而是提示动作has been completed，暗示need验证
    ...
    public String doubleClick(@P("Coordinate position array [x, y] in Gemini format (0-999)") int[] coords) {
        ...
        if (coords[0] < 0 || coords[0] > ScreenCapturer.COORD_MAX || 
            ...
            log.warn(" 坐标超出范围: [{}, {}] (有效范围: 0-{}), will 自动钳制", 
                    coords[0], coords[1], ScreenCapturer.COORD_MAX);
        }
        try {
            Point logical = toLogicalPoint(coords);
    ...
    public String scroll(@P("Scroll amount: positive down, negative up") int amount) {
        try {
            ...
            return "has been 执lines滚动操作。请检查可视区域是否更新。";
        } catch (Exception e) {
            return " 滚动exception: " + e.getMessage();
        }
    }
    ...
    public String type_at(String text) {
        try {
            ...
            return String.format("⌨️ 键盘敲击has been 发送: \"%s\"。请通过截图验证文characters是否上屏。", text);
        } catch (Exception e) {
            return " 输入exception: " + e.getMessage();
        }
    }

    /**
     * 在指定坐标Input text（1000x1000 Gemini 坐标系）
     *
     * 说明：
    ...
    public String type_text_at(
        ...
        try {
            ...
            // 3. Input text（不包含 Enter）
            robotDriver.type(text != null ? text : "");

            // 4. 返回提示info，强调不会自动按 Enter
            return String.format(
                    "⌨️ Performed at logical coordinates(%d, %d) Input text: \"%s\"（Gemini 坐标:%d,%d)。本工具不会自动按下 Enter, 如需提交请单独调用 keyCombination(\"enter\")。",
                    logical.x, logical.y, text, x, y
            );
        } catch (Exception e) {
            return " 坐标输入exception: " + e.getMessage();
        }
    }
    ...
    public String keyCombination(@P("Keyboard key or combination, e.g. 'enter', 'esc', 'tab', 'backspace', 'cmd+c', 'cmd+v', 'cmd+a', 'cmd+s', 'cmd+z', 'cmd+shift+p'") String keys) {
        if (keys == null || keys.isEmpty()) {
            return " Error: keys cannot 为空, for example 'enter' 或 'cmd+c'";
        }
        ...
        try {
            ...
            for (int i = 0; i < parts.length; i++) {
                ...
                if (code == null) {
                    return " 暂不支持的按键或别名: '" + token + "' 于组合 \"" + keys +
                            "\"。请使用常见写法, 如 cmd/ctrl/alt/shift + characters母/数characters/enter/esc/tab/backspace etc。";
                }
                keyCodes[i] = code;
            }
            ...
            if (keyCodes.length == 1) {
                ...
                return "has been 按下按键: " + normalized;
            } else {
                // 组合键：同时按下
                robotDriver.pressKeys(keyCodes);
                return "has been 发送组合键: " + normalized;
            }
        } catch (Exception e) {
            return " 按键/组合键执linesexception: " + e.getMessage();
        }
    }

    /**
     * will characters符串按键标识映射为 KeyEvent keyCode
     * 支持：
    ...
    private Integer mapKeyToken(String token) {
        ...
        // 单itemscharacters符处理：characters母、数characters、特殊characters符（大小写不敏感）
        if (token.length() == 1) {
            char c = token.charAt(0);
            
            // characters母 a-z 或 A-Z（大小写不敏感）
            if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
            ...
            // 特殊characters符：尝试通过characters符码获取
    ...
    public String openApplication(@P("Application name") String appName) {
        try {
            ...
            return result.success() ?
                    "has been 发送打开指令给: " + appName + "。请etc待UI加载。" :
    ...
    public String listInstalledApplications() {
        try {
            ...
            return result.success() ? "应用columns表:\n" + result.output() : " 获取columns表failed";
        } catch (Exception e) {
            return " exception: " + e.getMessage();
        }
    }
    ...
    public String quitApplication(@P("Application name") String appName) {
        try {
            ...
            return result.success() ? "has been 发送关闭指令。" : " 关闭failed: " + result.output();
        } catch (Exception e) {
            return " exception: " + e.getMessage();
        }
    }
    ...
    public String openURL(@P("URL address") String url) {
        try {
            ...
            return result.success() ? "has been 请求打开 URL: " + url + "。请检查浏览器是否has been 加载pages面。" : " Failed to open";
        } catch (Exception e) {
            return " exception: " + e.getMessage();
        }
    }
    ...
    public String openFile(@P("File path") String filePath) {
        try {
            ...
            return result.success() ? "has been 请求打开File: " + filePath : " Failed to open";
        } catch (Exception e) {
            return " exception: " + e.getMessage();
        }
    }
    ...
    public String revealInFinder(@P("Path") String filePath) {
        try {
            ...
            return result.success() ? "Performed at Finder 中选中。" : " 操作failed";
        } catch (Exception e) {
            return " exception: " + e.getMessage();
        }
    }
